...
export default function Home() {
  useEffect(() => {
    ...
    const fetchOnChainData = async () => {
      try {
        ...
        const stagePriceRaw = await presaleContract.stagePricesInUSD(stage - 1);
        setStagePriceUSD(Number(stagePriceRaw) / 1e18);
  ...
  const [stagePriceUSD, setStagePriceUSD] = useState<number | null>(null);
  ...
  const calculateTokensToBuy = async (
    bnbAmountInput: string
  ): Promise<number> => {
    try {
      ...
      const priceFromOracle = await presaleContract.getBNBUSDPrice(); // Chainlink price
      const currentStage = await presaleContract.currentStage();
      const stagePriceRaw = await presaleContract.stagePricesInUSD(
        currentStage - 1
      );

      const bnbUsdPriceOnChain = Number(priceFromOracle) / 1e8; // 8 decimals Chainlink
      const pricePerTokenInUSD = Number(stagePriceRaw) / 1e18; // 18 decimals USD price

      console.log(
        `Prezzo BNB/USD OnChain: $${bnbUsdPriceOnChain}, Prezzo token da contratto: $${pricePerTokenInUSD}`
      );

      const tokensToBuy =
        (Number(bnbAmountInput) * bnbUsdPriceOnChain) / pricePerTokenInUSD;

      console.log("TokensToBuy (calcolato):", tokensToBuy);

      return tokensToBuy;
    } catch (err: unknown) {
      console.error("Error calculating tokensToBuy:", err);
      return 0;
    }
  };
  ...
  const handleBuyWithBNB = async () => {
    ...
    try {
      ...
      const tokensToBuy = await calculateTokensToBuy(bnbAmount);
      console.log(
        "Tentativo acquisto:",
        tokensToBuy,
        "token con",
      ...
      // LOG di debug per capire quanti token ci sono disponibili
      const tokensForPublic = await presaleContract.tokensForPublic();
      const tokensDeposited = await presaleContract.tokensDeposited();
      console.log("TokensForPublic:", tokensForPublic.toString());
      ...
      try {
        ...
        console.log(
          "TokensAvailable:",
          (tokensForPublic - tokensSold).toString()
        );
      } catch (err) {
        console.log("Non esiste tokensSold() nel contratto?", err);
      }
        ...
        <section className="relative text-blue-900 text-center py-16 px-6 overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto rounded-xl shadow-lg border border-blue-300 bg-white p-8">
            ...
            <div className="bg-red-600 text-white py-3 px-6 rounded-xl shadow font-bold uppercase mb-8 text-lg">
              ...
              {isClient && stagePriceUSD !== null
                ? stagePriceUSD.toFixed(6)
                : "..."}
            </div>
